...
DUMMY_TEXT = `Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque ratione recusandae tempora tenetur quod repellat nihil. Repudiandae, sunt? Doloribus, laborum?`
...
$(function () {
    ...
    for (let i = 0, card_number = HEADING.length; i < card_number; i++) {
        document.querySelectorAll('section')[0].innerHTML += Home_page_card(HEADING[i], PARA[i]);
    }

    // Making cards for links
    for (let i=0, card_number = A_HEADING.length; i<card_number; i++)
    {
    ...
    for (let i=0; i<4; i++)
    {
        document.querySelector('footer').innerHTML += Profile_maker(PROFILE[`${i+1}`]["source"], PROFILE[`${i+1}`]["name"], PROFILE[`${i+1}`]["para"], PROFILE[`${i+1}`]["linkedin"], PROFILE[`${i+1}`]["email"]);
    }
...
{
    child = `<a href="${href}" class="col d-flex p-2 border border-primary shadow bg-info bg-gradient animate__animated animate__fadeInDown">
...
function Profile_maker(src, name, para, linkedin, email)
{
    child = `<section>
                <div class="row d-flex border shadow">
                    ...
                    <div class="border col d-flex">
                        <img src="${src}" class="card-img-left" alt="profile">
                    ...
                    <div class="container col d-flex">
                        ...
                        <div class="d-flex">
                            ...
                            <div>
                                ...
                                <a href="${linkedin}" class="btn btn-primary" role="button"
                                    target="_blank">LinkedIn</a>
                                <a href="${email}" class="btn btn-primary" role="button">Email</a>
